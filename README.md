...
[🚀 Quick Start](#-quick-start) • [📖 Documentation](#-documentation-links) • [🛡️ Security](#️-security) • [📊 Monitoring](#-monitoring)
...
## 🧰 PHP Apps That Need Writes (Archivarix CMS)

Some PHP apps (e.g., Archivarix CMS) need to write to the site folder. This project supports that safely:

- `php-fpm` mounts `./sites` as writable: `./sites:/srv/sites`
- On container start, you can optionally auto-fix ownership/perms so the app can write.

Enable auto-fix by setting in `.env`:

```bash
SITES_CHOWN_ON_START=true
SITES_DIR=/srv/sites
```
...
This removes the need for manual chown after deploys.
...
# 📝 PHP apps needing writes (optional)
